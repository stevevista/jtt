...
class Peer {
  ...
  emit(evt, ...args) {
    this.app.emitPeerEvent(evt, this, ...args)
  }
  ...
  heartBeat() {
      ...
      const interval = this.app.config.heartBeatInterval
      this.syncTimer = setTimeout(()=> {
        this.emit('lostHeartBeat', interval, (close) => {
  ...
  async handleMessage(app, msg) {
    ...
    switch(msg.messageId) {
      ...
      // auth
      case 0x0102: {
        const result = await this.onAuth(msg.token)
        this.authed = result === 0
        return this.ack(msg, result)
      }

      case 0x0100: {
        const [result, token] = await this.onRegister(msg)
        this.authed = result === 0
        return this.write(0x8100, {ackSN: msg.SN, result, token})
      }
      ...
      switch(msg.messageId) {
        ...
        case 0x0107: {
          ...
          this.emit('properties', msg);
          return
        }

        case 0x0200: {
          await this.onPosition(msg)
          return
        }

        case 0x0201: {
          const r = await this.onPosition(msg)
          return this.resolve(0x8201, msg.ackSN, r)
        }
            
        case 0x0500: {
          const r = await this.onPosition(msg)
          return this.resolve(0x8500, msg.ackSN, r)
        }

        case 0x0704: {
          const ret = {}
          copyMesageBody(ret, msg)
          await this.onBlulkPositions(ret)
          return this.ack(msg)
        }
        ...
        case 0x0800: {
          const ret = {}
          copyMesageBody(ret, msg)
          await this.onMediaEvent(ret)
          return this.ack(msg)
        }

            case 0x0700: {
              this.emit('drivingRecord', msg, (ret)=> {
            ...
            case 0x0702: {
                ...
                this.emit('driver', msg, next);
                return;
            }

            // multimedia uploading
        case 0x0801: {
          await this.emitMediaFile(msg)
          return
        }
    ...
    if (_.includes(AUTO_ACK_MSGS, msg.messageId)) {
      ...
      const ret = {}
      copyMesageBody(ret, msg)
      this.emit(messageName, ret)
      return true
    }

    this.emit(messageName, msg, (result)=> {
      this.ack(msg, result)
    })
  }
}
